...
export const createPost = async (content, mediaUrl, isHidden, ip) => {
  console.log("Server-side IP received for createPost:", ip); // Log IP on server-side
  try {
    const allowed = limit(ip, 3, 3_600_000); // 3 posts per hour per IP
    if (!allowed) {
      return {
        success: false,
...
export const updatePost = async (id, content, ip) => { // Added ip parameter
  console.log("Server-side IP received for updatePost:", ip); // Log IP on server-side
  try {
    const allowed = limit(ip, 5, 3_600_000); // 5 updates per hour per IP
    if (!allowed) {
      return {
        success: false,
        error: "Rate limit exceeded. Please try again later.",
